...
class engine_tests(unittest.TestCase):
    ...
    def testget_best_moveTest(self):
        ...
        self.chess.set_fen(
            "r1bqkbnr/pppp1Q1p/2n3p1/4p3/2B1P3/8/PPPP1PPP/RNB1K1NR b KQkq - 0 4")
        self.assertEqual(False, self.chess.get_best_move_depth(
            15), "Function returned false")

    def testset_fen_position(self):
        self.chess.start_new_game()
        Fen = self.chess.get_fen()
        self.assertEqual(
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", Fen, "Fen is wrong")
    ...
    def testget_evaluation_millis(self):
        self.chess.start_new_game()
        evaluationDepth = self.chess.get_evaluation_depth(self)
        self.assertGreaterEqual(1.0, evaluationDepth, "Evaluation is correct")
